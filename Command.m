...
classdef Command < handle
    ...
    methods
        ...
        function opINPUT_DEVICE_READY_RAW(cmd,layer,no,mode)
            % Command.opINPUT_DEVICE_CLR_ALL Add a opINPUT_DEVICE_CLR_ALL
            %
            % Command.opINPUT_DEVICE_CLR_ALL(layer) adds a opINPUT_DEVICE
            % opcode with a CLR_ALL subcode to the command object.
            ...
            cmd.addDirectCommand(ByteCodes.InputDevice);
            cmd.LC0(InputDeviceSubCodes.ReadyRaw);
            cmd.LC0(layer);
            cmd.LC0(no);
            cmd.LC0(0);
            cmd.LC0(mode);
        ...
        function opINPUT_DEVICE_SET_MODE(cmd,layer,no,mode)
        ...
        function opINPUT_READRAW(cmd,layer,no,type,mode,values)
            

            cmd.addDirectCommand(ByteCodes.InputReadExt);
            cmd.LC0(layer);
            cmd.LC0(no);
            cmd.LC0(type);
            cmd.LC0(mode);
            cmd.LC0(18); % Raw Format
            cmd.LC0(values);
            for i=0:(values-1)
                cmd.GV0(i*4);
            end
